...
##load data
dataset1 <- readRDS("DC1 full transcirpt dataset 1")
dataset2 <- readRDS("DC1 full transcirpt dataset 2")
dataset3 <- readRDS("DC1 full transcirpt dataset 3")
dataset4 <- readRDS("DC1 full transcirpt dataset 4")

##Four datasets were mixed firstly
library(BSgenome)
require(BSgenome.Hsapiens.UCSC.hg19)
library(Biostrings)
SiteData <- c(dataset1, dataset2, dataset3, dataset3)
SequenceData <- RNAStringSet(DNAStringSet(Views(Hsapiens, SiteData + 20)))
SequenceData.index <- list()
SequenceData.index[[1]] <- length(dataset1)
SequenceData.index[[2]] <- length(dataset2)
SequenceData.index[[3]] <- length(dataset3)
SequenceData.index[[4]] <- length(dataset4)
...
##In this part we show the evaluation process of using cross-sample testing
